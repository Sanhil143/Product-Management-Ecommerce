...
const createOrder = async function (req, res) {
  try {
    ...
    if (Object.keys(rest).length > 0)
      return res
        .status(400)
        .send({ status: false, message: "only provide cardId not any key" });
    if (!validObjectId(cartId))
      return res
        .status(400)
        .send({ status: false, message: "not valid card id " });
    ...
    if (cardDetails.items.length == 0)
      return res
        .status(400)
        .send({ status: false, message: "to add items in product" });
    ...
    if (cancellable) {
      if (cancellable || cancellable != "") {
        if (typeOf(cancellable) != "boolean") {
          return res
            .status(400)
            .send({ status: false, message: "please enter boolean value" });
        }
      }
    }
    ...
    let obj = {
      ...
      totalPrice: cardDetails.totalPrice,
      totalQuantity: sum,
      cancellable: data.cancellable,
    };
    let f = [];
    let createData = await OrderModel.create(obj);
    let d = await CartModel.findByIdAndUpdate(
      { _id: cardDetails._id },
      { $set: { items: f } },
      { new: true }
    );
  ...
  try {
    ...
    if (status != "pending" && status != "completed" && status != "cancelled") {
      return res
        ...
        .send({
          status: false,
          message: "order status can only be pending,completed and canceled",
        });
    }
    const findOrder = await OrderModel.findById({ _id: orderId });
    if (!findOrder)
      return res.status(404).send({ status: false, message: "oder Not found" });
    ...
    if (findOrder.status == "completed")
      return res
        ...
        .send({
          status: false,
          message: "Can Not Update This Order, Because It's Completed Already",
        });
    ...
    if (findOrder.status == "cancelled")
      return res
        ...
        .send({
          status: false,
          message: "Can Not Update This Order, Because It's Cancelled Already",
        });
