...
const createProduct = async function (req, res) {
  try {
    ...
    if (Object.keys(rest).length > 0) {
      return res
        .status(400)
        .send({ status: false, message: "another key not acceptable" });
    }
    ...
    if (!productImage)
      return res
        .status(400)
        .send({ status: false, message: "please enter productImage" });
    if (!availableSizes)
      return res
        .status(400)
    ...
    if (installments) {
      if (!isNum(installments))
        return res
          .status(400)
          .send({
            status: false,
            message: " installment only use number and two num use ",
          });
    }
    ...
    for (let i = 0; i < d.length; i++) {
      if (!sizeCheck.includes(d[i]))
        return res
          .status(400)
          .send({
            status: false,
            message: "at least one size like = S, XS,M,X, L,XXL,XL",
          });
      if (!x.includes(d[i])) {
        x.push(d[i]);
      }
    ...
    if (!(currencyId !== "INR" || currencyId !== "USD"))
      return res
        ...
        .send({
          status: false,
          message: "please enter valid currencyId like INR or USD ",
        });
    ...
    if (findTitle)
      return res
        .status(400)
        .send({ status: false, message: "title already exist our data base" });

    if (productImage && productImage.length === 0) {
      return res
        .status(400)
        .send({ status: false, message: "please insert product image!" });
    }
    if (productImage.length > 0) {
      if (productImage.length > 1)
        return res
          ...
          .send({
            status: false,
            message: "please select only one product image",
          });
      if (!validFile(productImage[0].originalname))
        return res
          ...
          .send({
            status: false,
            message: "please select valid  image like jpeg , png ,jpg",
          });
...
const getProductByFilter = async function (req, res) {
  try {
    ...
    if (size || size != undefined) {
      ...
      if (
        size.trim() != "" &&
        ["S", "XS", "M", "X", "L", "XXL", "XL"].includes(size.trim())
      ) {
        ...
        return res
          ...
          .send({
            status: false,
            message: "Size does not exists or Invalid size",
          });
      }
    }
    ...
    if (
      ...
      (priceGreaterThan != undefined && priceLessThan != undefined)
    ) {
      ...
      if (
        priceGreaterThan != "" &&
        priceLessThan != "" &&
        numValid(priceGreaterThan) &&
        numValid(priceLessThan)
      ) {
        ...
        return res
          ...
          .send({
            status: false,
            message: "Price must contain both the  range to filter data .",
          });
      }
      ...
      if (priceGreaterThan != "" && numValid(priceGreaterThan)) {
        ...
        return res
          ...
          .send({
            status: false,
            message:
              "Please provide range in price-greater than to filter data!",
          });
      }
    } else if (priceLessThan || priceLessThan != undefined) {
      priceLessThan = priceLessThan.trim();

      if (priceLessThan == "" && !numValid(priceLessThan)) {
        return res
          .status(400)
          .send({
            status: false,
            message: "Please provide range in price-less than to filter data!",
          });
      }
    ...
    if (priceSort || priceSort != undefined) {
      //{price:-1/1}
      ...
      if (priceSort == "" || (priceSort != -1 && priceSort != 1)) {
        return res
          ...
          .send({
            status: false,
            message: "Please provide -1 OR 1 for Price sort !",
          });
      }
      s.price = priceSort;
    }
    ...
    if (Data.length == 0) {
      return res
        .status(400)
        .send({ status: false, message: "Out of stock !!" });
    }
...
let updateProduct = async (req, res) => {
  try {
    ...
    if (!dataForUpdates || !productImage)
      return res
        .status(400)
        .send({
          status: false,
          message: "please provide some data for updating",
        });
      ...
      if (isTitleAlreadyUsed)
        return res
          ...
          .send({
            status: false,
            message:
              "This  is title already in use,please provide another title",
          });
      updateData.title = title;
    }

    if (description || description !== undefined) {
      if (description.trim() == "" || !validRegex(description))
        return res
          .status(400)
          .send({ status: false, message: "please provide valid description" });
    ...
    if (isFreeShipping || isFreeShipping !== undefined) {
      if (!(isFreeShipping == "true" || isFreeShipping == "false"))
        return res
          ...
          .send({
            status: false,
            message: "please provide valid isFreeShipping(true / false)",
          });
    ...
    if (installments || installments !== undefined) {
      ...
      if (!installments)
        return res
          ...
          .send({
            status: false,
            message: "please provide installments in digits",
          });
    ...
    if (price || price !== undefined) {
      ...
      if (!price)
        return res
          .status(400)
          .send({ status: false, message: "please provide price in digits" });
    ...
    if (!updatedProduct)
      return res
        .status(404)
        .send({ status: false, message: "Product not found to update" });
    return res
      .status(200)
...
const deleteProduct = async (req, res) => {
  try {
    ...
    if (Object.keys(productId).length === 0) {
      return res
        .status(400)
        .send({ status: false, message: "please enter product Id on param!" });
    }
    if (!validObjectId(productId)) {
      return res
        .status(400)
    ...
    let delData = await ProductModel.findByIdAndUpdate(
      { _id: productId, isDeleted: false },
    ...
    if (!delData) {
      return res
        .status(404)
        .send({ status: false, message: "product not found to be deleted" });
    }
    ...
    return res
      .status(200)
      .send({ status: true, message: "Success", data: delData });
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });
  }
};
