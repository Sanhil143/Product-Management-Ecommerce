  ...
  try {
    ...
    if (verifyEmail) {
      return res.status(400).send({
        status: false,
        message: "email is already exist! please enter another email address",
      });
    }
    ...
    if (verifyPhone) {
      return res.status(400).send({
        status: false,
        message:
          "phone number is already exist! please enter another phone number",
      });
    }
    ...
    try {
      ...
      if (!Array.isArray(add) == false) {
        return res.status(400).send({
          status: false,
          message: "inset address must be an object format",
        });
      }
      if (!add.shipping) {
        return res
      ...
      if (!add.shipping.street) {
        return res.status(400).send({
          status: false,
          message: "please enter street name on shipping address!",
        });
      }
      if (!validStreet(add.shipping.street)) {
        return res.status(400).send({
          status: false,
          message: "please enter valid street name on shipping address!",
        });
      }
      if (!add.shipping.city) {
        return res.status(400).send({
          status: false,
          message: "please enter city name on shipping address!",
        });
      }
      if (!validCity(add.shipping.city)) {
        return res.status(400).send({
          status: false,
          message: "please enter valid city name on shipping address!",
        });
      }
      if (!add.shipping.pincode) {
        return res.status(400).send({
          status: false,
          message: "please enter pincode on shipping address!",
        });
      }
      if (!validPinCode(add.shipping.pincode.trim())) {
        return res.status(400).send({
          status: false,
          message: "please enter valid pincode on shipping address!",
        });
      }
      if (!add.billing) {
        return res
      ...
      if (!add.billing.street) {
        return res.status(400).send({
          status: false,
          message: "please enter street name on billing address!",
        });
      }
      if (!validStreet(add.billing.street)) {
        return res.status(400).send({
          status: false,
          message: "please enter valid street name on billing address!",
        });
      }
      if (!add.billing.city) {
        return res.status(400).send({
      ...
      if (error) {
        return res.status(400).send({
          status: false,
          message: "please enter address in valid object format",
        });
      }
    }
    ...
    if (profileImage && profileImage.length === 0) {
      return res
        .status(400)
        .send({ status: false, message: "please insert profile image!" });
    }
    ...
    if (profileImage.length > 0) {
      if (profileImage.length > 1)
        return res.status(400).send({
          status: false,
          message: "please select only one profile image",
        });
      if (!validFile(profileImage[0].originalname))
        return res.status(400).send({
          status: false,
...
const login = async (req, res) => {
  try {
    ...
    if (Object.keys(data).length === 0) {
      return res
        .status(400)
        .send({ status: true, message: "Please enter email or password" });
    }
    ...
    let token = jwt.sign({ userId: findData._id }, process.env.Jwt_Secret, {
      expiresIn: "1h",
    });
...
const getUser = async (req, res) => {
  try {
    ...
    if (Object.keys(userId).length === 0) {
      return res
        .status(400)
        .send({ status: false, message: "Please enter userId on param" });
    }
    if (!validObjectId(userId)) {
      return res
        .status(400)
        .send({ status: false, message: "Please enter valid userId on param" });
    }
    ...
    return res
      .status(200)
      .send({ status: true, message: "User profile details", data: findData });
  } catch (error) {
    return res.status(500).send({ status: false, message: error, message });
  }
};
...
const updateUser = async (req, res) => {
  try {
    ...
    if (Object.keys(data).length === 0) {
      return res
        .status(400)
        .send({ status: false, message: "please provide key and value" });
    }
        ...
        if (billing) {
          ...
          if (city) {
            if (!validCity(city))
              return res.status(400).send({
                status: false,
                message: "please enter valid city name on billing address!",
              });
          ...
          if (pincode) {
            if (!validPinCode(pincode))
              return res.status(400).send({
                status: false,
                message: "please enter valid pincode on billing address!",
              });
    ...
    if (email) {
      if (!validMail(email)) {
        return res
          .status(400)
          .send({ status: false, message: "email invalid" });
      }
    ...
    if (phone) {
      if (!validNumber(phone)) {
        return res.status(400).send({
          status: false,
          message: " phone invalid use only 10 Number",
        });
      }
    ...
    if (files && files.length > 0) {
      ...
      if (!upload) {
        return res
          .status(400)
          .send({ status: true, message: "please insert profile image" });
      }
      data.profileImage = upload;
    }

    let updateData = await UserModel.findByIdAndUpdate({ _id: id }, data, {
      new: true,
    });
    if (!updateData) {
      return res
        .status(404)
        .send({ status: false, message: "not found user id " });
    }
