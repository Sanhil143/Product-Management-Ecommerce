  ...
  try {
    ...
    if (Object.keys(data).length == 0) {
      return res
        ...
        .send({
          status: false,
          message: "Put the productId you want to add to Cart",
        });
    }
    ...
    if (!cartId) {
      ...
      if (theCarts) {
        return res
          ...
          .send({
            status: false,
            message: `Cart is Already exists, put ${theCarts._id} this cartId in the body`,
          });
      }
      let cartCreate = {
        userId: userId,
      ...
      if (!isValidObjectId(cartId)) {
        return res
          .status(400)
          .send({ status: false, message: "cart id not valid id" });
      }
      ...
      if (!cartDetails) {
        return res
          ...
          .send({
            status: false,
            message: `Cart does not found, try with right cartId`,
          });
      }
      if (cartDetails.userId != userId) {
        return res
          ...
          .send({
            status: false,
            message: `You are not owner of this Cart, please try with ${cartDetails._id} this cartId`,
          });
      }
    }
    for (var i = 0; i < cartDetails.items.length; i++) {
      if (cartDetails.items[i].productId == productId) {
        cartDetails.items[i].quantity =
          cartDetails.items[i].quantity + quantity;
        break;
      }
    }
    ...
    cartDetails.totalPrice =
      cartDetails.totalPrice + productDetails.price * quantity;
...
const updateCart = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(productId)) {
      return res
        .status(400)
        .send({ status: false, message: "please insert valid productId!" });
    }
    let checkProduct = await ProductModel.findById({
      _id: productId,
      isDeleted: false,
    });
    if (!checkProduct) {
    ...
    if (!isValidObjectId(cartId)) {
      return res
        .status(400)
        .send({ status: false, message: "please insert valid cartId" });
    }
    ...
    if (checkCart.items.length === 0) {
      return res
        ...
        .send({
          status: false,
          message: "you have not added any product in your cart!",
        });
    }
    ...
    if (!(removeProduct == 0 || removeProduct == 1)) {
      return res
        ...
        .send({
          status: false,
          message: "please enter valid number it can be only 0 & 1",
        });
    }
    ...
    for (let i = 0; i < cart.length; i++) {
      if (cart[i].productId == productId) {
        ...
        if (removeProduct == 0) {
          let proRemove = await CartModel.findOneAndUpdate(
            { _id: cartId },
            {
          ...
          return res
            .status(200)
            .send({ status: true, message: "Success", data: proRemove });
        }
        if (cart[i].quantity == 1 && removeProduct == 1) {
          let proRemove1 = await CartModel.findOneAndUpdate(
            { _id: cartId },
            {
              $pull: { items: { productId: productId } },
              totalPrice: checkCart.totalPrice - finalPrice,
          ...
          return res
            .status(200)
            .send({ status: true, message: "Success", data: proRemove1 });
        }

        cart[i].quantity = cart[i].quantity - 1;
        let updatePro = await CartModel.findOneAndUpdate(
          { _id: cartId },
          {
            items: cart,
        ...
        return res
          .status(200)
          .send({ status: true, message: "Success", data: updatePro });
      }
    }
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
...
const getCart = async function (req, res) {
  try {
    ...
    if (!getCart) {
      return res
        .status(404)
        .send({ status: false, message: "Your cart is not found" });
    }
    return res
      .status(200)
...
const deleteCart = async function (req, res) {
  try {
    ...
    if (!noCart) {
      return res
        .status(404)
        .send({ status: true, message: "Cart not found for the user  " });
    }

    let a = [];
    if (noCart.totalItems > 0) {
      let deleted = await CartModel.findOneAndUpdate(
        { userId: userId },
        { items: a, totalPrice: 0, totalItems: 0 },
        { new: true }
      ).select({ __v: 0, createdAt: 0, updatedAt: 0 });

      return res
        .status(200)
        .send({ status: true, message: "Cart deleted", data: deleted });
    }
    ...
    return res
      ...
      .send({
        status: true,
        message: "Data of this cart has already been deleted !",
      });
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
  }
